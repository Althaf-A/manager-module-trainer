...
def trainer_applyleave(request):
    if 'tr_id' in request.session:
        ...
        return render(request,'software_training/training/trainer/trainer_applyleave.html',{'z': z})
    else:
        return redirect('/')
...
def trainer_applyleave_form(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        le = user_registration.objects.get(id=tr_id)
        des1 = designation.objects.get(designation_name='trainer')
        if request.method == 'POST':
            mem = leave()
            mem.leave_from_date = request.POST['from']
            mem.leave_to_date = request.POST['to']
            mem.leave_status = request.POST['haful']
            mem.leave_reason = request.POST['reason']
            mem.leave_user = le
            mem.leave_designation_id = des1.id
            mem.leave_leaveapproved_status=0       
            mem.save()
            return render(request, 'software_training/training/trainer/trainer_applyleave.html', {'z': z})
        return render(request, 'software_training/training/trainer/trainer_applyleave_form.html', {'z': z})
    else:
        return redirect('/')
...
def trainer_traineesleave_table(request):
    if 'tr_id' in request.session:
        ...
        tm = leave.objects.filter(leave_designation_id=des.id) .all().order_by('-id')
        return render(request, 'software_training/training/trainer/trainer_traineesleave_table.html', {'tm': tm, 'z': z})
    else:
        return redirect('/')
...
def trainer_reportissue(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        return render(request,'software_training/training/trainer/trainer_reportissue.html',{'z': z})
    else:
        return redirect('/')
...
def trainer_reportissue_form(request):
    if 'tr_id' in request.session:
        ...
        mem = reported_issue()
        des = designation.objects.get(designation_name='manager')
        cut = user_registration.objects.get(designation=des.id)
        mem1 = user_registration.objects.get(id=tr_id)
        des1 = designation.objects.get(designation_name='trainer')
        print(mem1)
        if request.method == "POST":
            mem.reported_issue_issue = request.POST['issues']
            mem.reported_issue_reported_date = datetime.now()
            mem.reported_issue_reported_to = cut
            mem.reported_issue_reporter = mem1
            mem.reported_issue_designation_id = des1
            mem.reported_issue_issuestatus =0
            mem.save()
            return render(request, 'software_training/training/trainer/trainer_reportissue.html', {'z': z})
        return render(request, 'software_training/training/trainer/trainer_reportissue_form.html', {'mem': mem, 'z': z})
    else:
        return redirect('/')
...
def trainer_reportedissue_table(request):
    if 'tr_id' in request.session:
        ...
        z = user_registration.objects.filter(id=tr_id)
        des = designation.objects.get(designation_name='trainee')
        cut = reported_issue.objects.filter(reported_issue_reported_to_id=tr_id).filter(reported_issue_designation_id=des.id).filter(reported_issue_issuestatus=0).order_by('-id')
        return render(request,'software_training/training/trainer/trainer_reportedissue_table.html',{'cut': cut, 'vars': vars,  'z': z})
    else:
        return redirect('/')

def savereplaytnee1(request, id):
    if request.method == 'POST':
        vars = reported_issue.objects.get(id=id)
        vars.reported_issue_reply = request.POST['reply']
        vars.reported_issue_issuestatus = 1
        vars.save()
    return redirect('/softwareapp/trainer_reportedissue_table')
...
def trainer_myreportissue_table(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        rm = reported_issue.objects.filter(reported_issue_reporter_id=tr_id).order_by('-id')
        return render(request,'software_training/training/trainer/trainer_myreportissue_table.html',{'rm':rm ,'z': z})
    else:
        return redirect('/')
...
def trainer_attendance(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        return render(request,'software_training/training/trainer/trainer_attendance.html',{'z': z})
    else:
        return redirect('/')
...
def trainer_attendance_trainees(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        return render(request,'software_training/training/trainer/trainer_attendance_trainees.html',{'z': z})
    else:
        return redirect('/')
...
def trainer_attendance_trainer(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        return render(request,'software_training/training/trainer/trainer_attendance_trainer.html',{'z': z})
    else:
        return redirect('/')
...
def trainer_attendance_trainer_viewattendance(request):
    if 'tr_id' in request.session:
        if request.session.has_key('tr_id'):
           tr_id = request.session['tr_id']
        z = user_registration.objects.filter(id=tr_id)
        return render(request, 'software_training/training/trainer/trainer_attendance_trainer_viewattendance.html',{'z':z})
    else:
        return redirect('/')
...
def trainer_attendance_trainer_viewattendancelist(request):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            atten = attendance.objects.filter(attendance_date__gte=std,attendance_date__lte=edd,attendance_user_id=tr_id).order_by('-id')
        return render(request, 'software_training/training/trainer/trainer_attendance_trainer_viewattendancelist.html',{'z':z,'atten':atten})
    else:
        return redirect('/')
...
def trainer_attendance_trainees_viewattendance(request):
    if 'tr_id' in request.session:
        ...
        return render(request, 'software_training/training/trainer/trainer_attendance_trainees_viewattendance.html',{'z':z,'vars':vars})
    else:
        return redirect('/')
...
def trainer_attendance_trainees_viewattendancelist(request):
    if 'tr_id' in request.session:
        ...
        return render(request, 'software_training/training/trainer/trainer_attendance_trainees_viewattendancelist.html',{'z':z,'attend':attend})
    else:
        return redirect('/')
...
def trainer_attendance_trainees_addattendance(request):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            atten = attendance()
            a=request.POST['sele']
            b=user_registration.objects.get(id=a)
            atten.attendance_date = request.POST['date']
            atten.attendance_user = b
            atten.attendance_status = request.POST['pres']
            atten.save()
        return render(request, 'software_training/training/trainer/trainer_attendance_trainees_addattendance.html',{'z':z,'vars':vars})
    else:
        return redirect('/')
